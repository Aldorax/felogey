...
interface InternComponentProps {
  user: User;
}

const InternComponent: React.FC<InternComponentProps> = ({ user }) => {
  return (
    <div className="max-w-screen">
      ...
      <Card className="w-[90vw] p-4">
        <CardHeader className="flex gap-3">
          <div className="flex flex-col">
            <p className="text-md">Reserved Wallet Details</p>
            <p className="text-small text-default-500 font-semibold">
              Enetworksagencybanking.com.ng
            </p>
          </div>
        ...
        <CardFooter className="flex justify-between">
          <div>Balance</div>
          <div>&#x20A6;{user?.reserved_earnings}</div>
        </CardFooter>
      </Card>
      <br />
      <Card className="w-[90vw] p-4">
        ...
        <CardFooter className="flex flex-col">
          ...
          <Divider />
          <div className="flex justify-between w-full mt-2 mb-2">
            <div>Total No of Agents Referred</div>
            <div>{user?.total_agents_referred}</div>
          </div>
          <Divider />
          <div className="flex justify-between w-full mt-2 mb-2">
            <div>Total No of Interns Referred</div>
            <div>{user?.total_interns_referred}</div>
          </div>
          <Divider />
          <div className="flex justify-between w-full mt-2 mb-2">
